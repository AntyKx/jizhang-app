...
// Single source of truth for what a backup contains, shared by the manual
// download (/api/export/backup) and the nightly snapshot the cron job
// writes to Blob. Keeping one builder is the point: a snapshot that drifts
// from what restoreBackup() expects is worse than no snapshot at all,
// since it only fails at the moment someone is actually trying to recover.
