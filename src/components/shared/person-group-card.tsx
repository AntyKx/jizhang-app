...
// One counterparty's unsettled items, collapsed to a single card — settling
// with them is one tap here instead of hunting down every item that
// mentions their name individually.
export function PersonGroupCard({
  ...
  function handleSettleAll(e: React.MouseEvent) {
    e.stopPropagation();
    startTransition(async () => {
      const result = await settleAllForName(name, { from: dateFrom, to: dateTo });
      if (isFail(result)) {
    ...
    <Collapsible open={open} onOpenChange={setOpen} className="rounded-2xl border bg-card">
      ...
      <CollapsiblePanel>
        ...
        <div className="flex justify-end px-4 py-2.5">
          <Button size="sm" variant="secondary" disabled={pending} onClick={handleSettleAll}>
            {pending ? "處理中…" : "全部結清"}
          </Button>
        </div>
      </CollapsiblePanel>
